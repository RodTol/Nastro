...
${HOME}/Nastro/Basecalling_pipeline/launch_run/server.sh $model $logs_dir/server_node_$node_name $gpus_settings $dorado_port &
SERVER_PID=$!
...
# while true; do
#     output=$(python3 ${HOME}/Nastro/Basecalling_pipeline/launch_run/check_icp_port.py ${port_file})
#     if [[ "$output" == *"True"* ]]; then
...
exec python3 ${HOME}/Nastro/Basecalling_pipeline/launch_run/ParaCall/BCProcessors.py $json_file $my_index $dorado_port >> $BC_processor_log_path 2>&1 &
BC_PROCESSOR_PID=$!

#TODO the version hardcoded is ridicolous XD
is_ready=$(python3 ${HOME}/Nastro/Basecalling_pipeline/launch_run/check_log_file.py $BC_processor_log_path "ONT basecaller supervisor version 7.4.12+0e5e75c49")
if [[ "$is_ready" == *"True"* ]]; then
...
python3 ${HOME}/Nastro/Basecalling_pipeline/launch_run/ParaCall/BCController.py $run_params_path $BC_MANAGER_PID $BC_PROCESSOR_PID $SERVER_PID $samplesheet >> "$BC_controller_log_path" 2>&1 &
